...
const config = {
    ...
    // ALTERADO: totalSupply agora é 79.000.000.000 (79 bilhões)
    // Market Cap = 79.000.000.000 × $10 = $790.000.000.000
    totalSupply: parseInt(process.env.TOTAL_SUPPLY) || 79000000000,
...
console.log(`   ⛏️  Miner Address: ${config.minerAddress}`);
console.log(`   💰 Total Supply: ${config.totalSupply.toLocaleString()} BRD`);
console.log(`   💵 Market Cap (a $10): $${(config.totalSupply * 10).toLocaleString()} USD`);
...
// ============================================
// NOVAS ROTAS PARA COINGECKO
// ============================================

// ============================================
// ROTA: /address/:address - Informações da Wallet
// ============================================
app.get('/address/:address', (req, res) => {
    try {
        ...
        const transactions = blockchain.history
...
// ============================================
// ROTA: /api/rewards - Bloco de Recompensa
// ============================================
app.get('/api/rewards', (req, res) => {
    try {
        const stats = blockchain.getStats();
        const latestBlock = blockchain.getLatest();
        
        const coinbaseTx = latestBlock.transactions.find(t => t.type === 'coinbase' || t.type === 'genesis');
        
        res.json({
            success: true,
            data: {
                chain: config.coinName,
                symbol: config.coinSymbol,
                chainId: config.chainId,
                network: config.network,
                
                current_block: latestBlock.index,
                block_hash: latestBlock.hash,
                block_timestamp: latestBlock.timestamp,
                
                block_reward: coinbaseTx ? coinbaseTx.amount : 0,
                base_reward: blockchain.calculateReward(latestBlock.index),
                total_fees: latestBlock.transactions.reduce((sum, tx) => sum + (tx.fee || 0), 0),
                
                validator: coinbaseTx ? coinbaseTx.to : config.minerAddress,
                miner_address: config.minerAddress,
                
                total_supply: config.totalSupply,
                circulating_supply: stats.circulating || 0,
                total_staked: stats.staked || 0,
                active_wallets: stats.activeWallets || 0,
                pending_transactions: stats.pending || 0,
                validators: stats.validators || 0,
                nodes_online: stats.nodesOnline || 1,
                
                block_time: config.blockTime || 10,
...
// ============================================
// ROTA: /api/supply - Supply da Moeda
// ============================================
app.get('/api/supply', (req, res) => {
    try {
        const stats = blockchain.getStats();
        const totalStaked = Object.values(blockchain.staking).reduce((s, v) => s + (v.staked || 0), 0);
        
        res.json({
            success: true,
            data: {
                total_supply: config.totalSupply,
                circulating_supply: stats.circulating || (config.totalSupply - totalStaked),
                staked_supply: totalStaked,
                burned_supply: 0,
                currency: config.coinSymbol,
                chain: config.coinName,
                timestamp: new Date().toISOString()
...
app.get('/explorer', (req, res) => {
    ...
    <body>
        ...
        <div class="stats">
            ...
            <div class="stat-item">Total Supply: <span>${stats.total.toLocaleString()} BRD</span></div>
        ...
        <div class="nav">
            ...
            <a href="/api/rewards">💰 Block Reward (JSON)</a>
        </div>
        ...
        <p style="color:#666;font-size:12px;">🔗 Para ver uma wallet: <a href="/address/${config.minerAddress}" style="color:cyan;">/address/${config.minerAddress}</a></p>
    </body>
    </html>
    `);
});
...
const server = app.listen(config.port, config.host, () => {
    ...
    console.log('   GET  /address/:address      - Wallet info (JSON) ⭐ NOVA');
    console.log('   GET  /api/rewards           - Block reward data (JSON) ⭐ NOVA');
    console.log('   GET  /api/supply            - Supply data (JSON) ⭐ NOVA');
